...
void binary_places_to_decimal(long, int, int);
...
char array[15];
char sign = positive;											//Default value
char signL;														//Ayways positive: Required when entering num decimal places to be printed out


FPN_LF = *(unsigned long*)&FPN;									//The print out routine treats the FPN as an unsigned long integer

if (FPN_LF & (0x80000000)) {sign = negative;	                 //Check for a negative sign                  
FPN_LF &= (~(0x80000000));}                                     //Clear sign bit

Serial.write ("     No. decimal places?    ");
Scientific_num_from_KBD(array,&signL);                       	//Enter number of decimal places required
num_decimal_places = atol(array);
...
(binary_places, expt_2, num_decimal_places);}					//Prints out binary places in decimal
...
/********************************************************************************************************/
void binary_places_to_decimal(long binary_points, int expt, int Num_dps){
long denominator = 1;                                                       
